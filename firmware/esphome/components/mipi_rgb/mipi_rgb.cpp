...
void MipiRgb::common_setup_() {
  ...
  // Local override (ColorSplash-XG project): the upstream ESPHome
  // copy of this file uses num_fbs=1, which on ESP32-S3 + PSRAM
  // framebuffer + concurrent Wi-Fi/BLE traffic produces visible
  // RGB-panel underrun glitches even at idle. Two framebuffers
  // enable the canonical ESP-IDF tear-free recipe: DMA reads from
  // the front buffer while CPU writes to the back, and they swap
  // on vsync. Cost: an extra 480×800×2 = 768 KB of PSRAM, well
  // Settings below match Waveshare's official ESP-IDF reference for
  // this board family (their `rgb_lcd_port.c`):
  //   - num_fbs = 2: double-buffer; DMA reads front, CPU writes
  //     back, swap on vsync — eliminates tearing.
  //   - bounce_buffer_size_px = width * 10: 10-line scratch in
  //     internal SRAM; DMA pre-fetches from PSRAM in chunks.
  //   - bb_invalidate_cache = 1: flush CPU cache after PSRAM
  //     writes so DMA reads fresh lines (PSRAM is cached on S3).
  //   - psram_trans_align = 64: align DMA transactions to the
  //     64-byte cache line; without this, bursts that straddle
  //     a cache line trigger extra fetches.
  //
  // Upstream ESPHome 2026.4.0 hardcodes num_fbs=1 and leaves
  // bb_invalidate_cache + psram_trans_align unset, which produces
  // visible tearing on this board family with concurrent BLE/Wi-Fi.
